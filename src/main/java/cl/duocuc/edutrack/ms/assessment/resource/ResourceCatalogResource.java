...
/**
 * Metadatos de auto-descripción del Assessment Service (contrato transversal
 ...
 * {@link AssessmentResourceId}. Público tras el Gateway (sin {@code @RequirePermission}).
 */
@Path("/meta/resources")
...
public class ResourceCatalogResource {
    ...
    private static final List<String> RESOURCE_KEYS = List.of(
        AssessmentResourceId.EVALUATIONS,
        AssessmentResourceId.GRADES,
        AssessmentResourceId.AUDIT
    );
    ...
    public DataResponse<List<String>> resources() {
        return DataResponse.of(RESOURCE_KEYS)
            .with("service", ServiceIds.ASSESSMENT)
